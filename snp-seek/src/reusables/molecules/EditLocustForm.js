...
function EditLocustForm({handleUpdateSuccess, listDetails,  selectedListContent, locusListsNames}) {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data) {
          ...
          let fetchGeneNames = await api.post(`/api/geneloci/getGeneNamesByIds`, {
            items: response.data
          });
  
          if (fetchGeneNames.data) {
            // Store it
            setStoredLines(fetchGeneNames.data.map(name => name.toUpperCase()))
          }
        }
      } catch (error) {
  ...
  const validate = () => {
    const validNames = locusListsNames.filter(name => name !== listDetails.name)
    let isValid = true;
    const newErrors = {};
