...
import EditSnpListForm from '../molecules/EditSnpListForm';
...
const SnpListModal = ({
  ...
  if (!isModalOpen || activeTab !== 'SNP' || !selectedList) return null;
  ...
  const handlePageChange = async (pageNumber) =>{
    setCurrentPage(pageNumber)
      try{
        ...
        if(response.data){
          setSelectedListContent(response.data.content);
        }
      } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 transition-opacity duration-300 ease-out animate-fadeIn">
      ...
      >
        <button 
          onClick={() => {setIsModalOpen(false); setsShowSnpEditForm(false)}} 
        ...
        <div className="w-full flex items-center mb-2 ">
          ...
          <div className="flex w-1/2 items-center justify-end mr-10 gap-2">
            <button
              onClick={() => setsShowSnpEditForm((prev) => !prev)}
              className={`px-2 py-2 w-32  text-white  shadow-md hover:text-yellow-1 transition font-['Open-Sans'] duration-300 ${showSnpEditForm ? "bg-red-800" : "bg-green-2 "}`}
            >
              {showSnpEditForm ? "Cancel Edit" : "Edit"}
            </button>
            {!showSnpEditForm && (
              <button
        ...
        {showSnpEditForm && (
          <EditSnpListForm
            ...
            snpSets={snpSets}
            snpListsNames={snpLists.map(list => list.name)}
          />
        )}
          ...
          <>
            <div className="bg-gray-100 p-4 rounded-lg shadow-md space-y-2">
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <span className="font-['Lato-Regular'] text-gray-700">
                  <strong className="text-green-1">SNP Set:</strong> {selectedList.snpSet}
                </span>
                <span className="font-['Lato-Regular'] text-gray-700">
            ...
            <div className="mt-2 relative overflow-auto shadow-md sm:rounded-lg max-h-[70%]">
                <table className="w-full text-sm text-left rtl:text-right text-gray-300">
                    ...
                    <tr>
                        <th scope="col" className="px-6 py-3 w-[16%]">Chromosome</th>
                        <th scope="col" className="px-6 py-3 w-[8%]">Position</th>
                    </tr>
                    </thead>
                    <tbody>
                    {selectedListContent?.map((item, index) => {
                        const [chromosome, position] = item.split(" "); // Splitting into chromosome and position
                        return (
                        <tr key={index} className="border-b bg-gray-800 border-gray-700 hover:bg-gray-600">
                            <th scope="row" className="px-6 py-4 font-medium whitespace-nowrap text-white">
                            {chromosome}
                            </th>
                            <td className="px-6 py-4">{position}</td> {/* Fallback if missing */}
                        </tr>
                        );
                    })}
                    </tbody>
